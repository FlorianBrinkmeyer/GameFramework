...
type NegaMaxTimeLimitedCaching (player : int, searchTime : int, ?maxCachedStates : int) =
    ...
    interface AI_Agent with
        ...
        member x.MakeMove mutableGame game =
            ...
            let timeLimitedSearch (state : ImmutableGame) =
                    ...
                    let rec helper step (state : ImmutableGame) =
                        ...
                        if (step = 0) || (numberOfPossibleMoves = 0)  then
                            ...
                            | _ ->
                                ...
                                value
                    chosenMove <- [0..(state.NumberOfPossibleMoves-1)] |> List.maxBy (fun move -> -(helper (searchDepth-1) (state.NthMove move)))
                    searchDepth <- searchDepth + 1
                    sendMessage.Trigger (sprintf "Reached search depth: %A" searchDepth)
            let state = game :?> ImmutableGame
            let thread = Thread (fun () -> timeLimitedSearch state)
            thread.Start ()
