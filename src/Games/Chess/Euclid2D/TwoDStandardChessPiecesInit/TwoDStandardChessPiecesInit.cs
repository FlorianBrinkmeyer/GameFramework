...
{
    public static IPiece Init (String kind, int color)
    {
        IPiece? result = null;
        switch (kind)
        {
            case "Pawn":
                if (color==1)
                    result = new Pawn<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>> 
                        ...
                        1, 0, FSharpOption<FSharpSet<Tuple<int, int>>>.None, FSharpOption<FSharpSet<Tuple<int, int>>>.None, Init ("Queen", 1));
                else
                    result = new Pawn<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>> 
            ...
            case "King":
                if (color==1)
                    result = new King<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>>
                        (TwoDMoveCalculation.whiteKingStandardMoves, TwoDMoveCalculation.kingWhiteCastling, 1, true, 
                        SetModule.Empty<Tuple<int, int>> (),
                        (BlockablePiece<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>>) Init ("Rook", 1));

                else
                    result = new King<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>>
                        (TwoDMoveCalculation.blackKingStandardMoves, TwoDMoveCalculation.kingBlackCastling, -1, true, 
                        SetModule.Empty<Tuple<int, int>> (),
                        (BlockablePiece<ImmutableEnumerable2DArray.ImmutableEnumerable2DArray<IPiece>, Tuple<int, int>>) Init ("Rook", -1));
            break;
        }        
